...
log = logging.getLogger("ai.stt.gemini")


class GeminiTranscriber(SpeechTranscriber):
    def __init__(
        self,
        model: str,
        ...
        if not api_key:
            raise ValueError("GEMINI_API_KEY is required for Gemini STT")
        self._types = types
    ...
    def _run_step(self, step: StepConfig, part, context: dict[str, str]) -> str:
        ...
        if not system_prompt and not user_prompt:
            user_prompt = "Transcribe the audio to plain text. Return only the transcription."
    ...
    def transcribe(self, path: Path, mime_type: str | None = None) -> str:
        try:
            ...
            if not self.steps:
                prompt = "Transcribe the audio to plain text. Return only the transcription."
                resp = self.client.models.generate_content(
                    model=self.model,
            ...
            log.warning("Gemini transcription failed | path=%s | error=%s", path, exc)
            return ""
